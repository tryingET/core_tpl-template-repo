...
| Release automation (release-please + publish) | L1/L2 (public package repos) | ✅ (optional via `enable_release_pack`, default `false`) | ✅ | Keep optional-by-profile; enable where release governance requires it.
...
| Security/release docs (`SECURITY.md`, release checks) | L1/L2 public | ✅ (optional via `enable_release_pack`, default `false`) | ✅ | Keep optional-by-profile for public release surfaces.
...
- **Main remaining differences** from `pi-extensions/template`: ecosystem-specific helpers (docs discovery wrappers, startup intake flows) and optional deeper contract-style path scanners.
