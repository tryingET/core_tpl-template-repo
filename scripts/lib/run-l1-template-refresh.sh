...
usage() {
	cat <<'EOF' >&2
usage: run-l1-template-refresh.sh <target> [repo-slug] [--bootstrap-map] [--apply] [evidence-ref]

Internal renderer/planner shared by the non-mutating preview and explicit
...
[ "$bootstrap_flag" != "--bootstrap-map" ] || set -- "$@" --bootstrap-map
[ "$apply_mode" != "1" ] || set -- "$@" --apply
[ -z "$evidence_ref" ] || set -- "$@" --evidence-ref "$evidence_ref"
[ -z "$plan_sha256" ] || set -- "$@" --plan-sha256 "$plan_sha256"
[ -z "$wave_id" ] || set -- "$@" --wave-id "$wave_id"
[ -z "$source_l0_commit" ] || set -- "$@" --source-l0-commit "$source_l0_commit"
"$python_exec" "$ownership_engine" "$@"

if [ "$apply_mode" != "1" ] && repo_surface_is_git_repo "$target_repo"; then
...
echo "==> diagnostic whole-tree comparison (non-actionable; ownership plan above governs apply)"
