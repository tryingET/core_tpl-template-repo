...
- AK CLI: `ak <ak args...>` (canonical operator path for repo-local AK work-items projection and task-scope snapshot flows when in scope; if a generated repo still ships `./scripts/ak.sh`, treat it as the launcher implementation behind plain installed `ak`)
