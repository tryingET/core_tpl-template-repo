---
summary: "Package-local override notes for the shared engineering-core lane used by this package."
read_when:
...
- `docs/engineering.local.md` records package-local deltas.
...
- If the upstream lane ships a conditional `engineering-<lane>.ts-quality.md` addendum and this package adopts `ts-quality`, keep package-local rollout truth in `docs/project/ts-quality-current-vs-target.md` and keep the detailed adoption doctrine upstream in the `ts-quality` repo.