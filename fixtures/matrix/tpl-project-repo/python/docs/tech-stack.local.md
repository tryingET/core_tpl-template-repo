...
- If the upstream lane ships a conditional `tech-stack-<lane>.ts-quality.md` addendum and this repo adopts `ts-quality`, keep repo-local rollout truth in `docs/dev/ts-quality-current-vs-target.md` and keep the detailed adoption doctrine upstream in the `ts-quality` repo.